...
def main():
    ...
    a("")
    a("The table scores only the graded prediction and parameter gates, because")
    a("the variants do not write a posterior file. Four entries need a note.\n")
    a("* `wrong_mass_basis` is not caught by the skill gates: the free parameters")
    a("  absorb most of a mis-scaled non-road source term. It is caught by the")
    a("  exported-inventory gate. Completing that variant with the posterior file")
    a("  such a solver would write, and running the sealed verifier on the whole")
    a("  triple, gives reward 0 with `total_source_flux does not equal the")
    a("  corrected road flux plus the prescribed non-road sources (max relative")
    a("  deviation 2.456e-01); check the nitrogen mass basis of fixed_sources`.")
    a("* `clipped_negatives` is genuinely harmless at this signal level and is")
    a("  reported as such rather than claimed as a trap.")
    a("* The grid convergence angle spans about six degrees across the domain, so")
    a("  ignoring it, or taking it with the wrong sign, is largely absorbed by the")
    a("  free rotation parameter. Both variants pass. The convention is documented")
    a("  because it is needed for a correct model, not because it is a trap.")
    a("* `first_order_upwind` and `coarse_time_step_300s` both fail. `model_spec.md`")
    a("  states in advance that a first-order scheme and a time step above 60 s are")
    a("  inadequate on the 4 km grid, so neither was used to set a threshold; they")
    a("  fail on their own merits.\n")
